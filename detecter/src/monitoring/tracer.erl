...
-spec start(PidS, MfaSpec, AMode, Parent) -> pid() when
    PidS :: pid(),
    MfaSpec :: analyzer:mfa_spec(),
    AMode :: a_mode(),
    Parent :: parent().
start(PidS, MfaSpec, AMode, Parent) when
    is_pid(PidS),
    is_function(MfaSpec, 1),
    is_pid(Parent);
    Parent =:= self
->
...
-spec root(PidS, MfaSpec, AMode, Starter, Parent) -> no_return() when
    PidS :: pid(),
    MfaSpec :: analyzer:mfa_spec(),
    AMode :: a_mode(),
    Starter :: pid(),
...
root(PidS, MfaSpec, AMode, Starter, Parent) ->
    ...
    State = #state{
        traced = add_proc(PidS, ?T_MODE_DIRECT, #{}),
        mfa_spec = MfaSpec,
        a_mode = AMode
    },
...
%%
%% {@returns Does not return.}
-spec tracer(PidS, PidT, AnlFun, MfaSpec, AMode, Parent) -> no_return() when
    PidS :: pid(),
    PidT :: pid(),
...
tracer(PidS, PidT, AnlFun, MfaSpec, AMode, Parent) ->
    ...
    State = #state{
        traced = add_proc(PidS, ?T_MODE_PRIORITY, #{}),
        mfa_spec = MfaSpec,
        a_mode = AMode
    },
...
%%
%% {@returns Does not return.}
-spec loop(TMode, State, Analyzer, Parent) -> no_return() when
    TMode :: t_mode(),
    State :: state(),
    Analyzer :: analyzer(),
    Parent :: parent().
...
%%  ?exec_if_test(show_state(State, ?MODE_DIRECT), ok),
  ...
  receive
    ...
    Cpt when element(1, Cpt) =:= corrupt_payload ->
      ?TRACE("(o) Obtained corrupt event ~w.", [Cpt]),

      % Corrupt trace event. It is handled by the tracer.
      State0 = handle_corrupt(?T_MODE_DIRECT, State, Cpt, Analyzer, Parent),
      loop(?T_MODE_DIRECT, State0, Analyzer, Parent);
...
-spec handle_event(TMode, State, Msg, Analyzer, Parent) -> state() when
    TMode :: t_mode(),
    State :: state(),
...
handle_event(
    ?T_MODE_PRIORITY,
    State,
    Rtd = {route, PidRtr, Evt = {trace, PidSrc, spawn, PidTgt, _}},
    Analyzer,
    Parent
) ->
    do_handle(
...
handle_event(
    ?T_MODE_PRIORITY,
    State,
    Rtd = {route, _PidRtr, Evt = {trace, PidSrc, exit, _}},
    Analyzer,
    Parent
) ->
    do_handle(
...
handle_corrupt(?T_MODE_DIRECT, State, Evt, Analyzer, _) when
  element(1, Evt) =:= corrupt_payload ->
  PidSrc = element(2, Evt),
...
-spec instr(TMode, State, Evt, PidT, Parent) -> state()
  ...
  PidT :: pid(),
  Parent :: parent().
instr(Mode, State, {trace, _, spawn, PidTgt, Mfa = {_, _, _}}, PidT, Parent) when
    Mode =:= ?T_MODE_PRIORITY; Mode =:= ?T_MODE_DIRECT
->
    ...
    case (State#state.mfa_spec)(Mfa) of
        undefined ->
            ...
            if
                Mode =:= ?T_MODE_PRIORITY -> detach(PidTgt, PidT);
                true -> ok
            end,
...
%%           collected.
%% }
-spec route_detach(State, Cmd, Analyzer, Parent) -> state() | no_return() when
    State :: state(),
    Cmd :: detach(),
    Analyzer :: analyzer(),
    Parent :: parent().
...
%% {@returns Updated tracer state, or does not return if tracer is garbage
%%           collected.}
-spec forwd_detach(State, Rtd, Analyzer, Parent) -> state() | no_return() when
    State :: state(),
    Rtd :: routed(detach()),
    Analyzer :: analyzer(),
    Parent :: parent().
...
%% {@returns Updated tracer state, or does not return if tracer is garbage
%%           collected.}
-spec handle_detach(State, Rtd, Analyzer, Parent) -> state() | no_return() when
    State :: state(),
    Rtd :: routed(detach()),
    Analyzer :: analyzer(),
    Parent :: parent().
    ...
    do_handle(
        ...
        fun _CheckIfCanTransitionToDirectMode() ->
            ...
            ?assertEqual(
                self(),
                Self,
                format("Tracer ~w is not equal to ~w for detach command", [Self, self()])
            ),
...
-spec forwd_event(State, Rtd) -> state() when
    State :: state(),
...
forwd_event(State, Rtd = {route, _, {trace, PidSrc, spawn, PidTgt, _}}) ->
    do_handle(
        ...
        fun _Fail() ->
            ...
            ?assert(
                false,
                format("Routed trace event ~w cannot be handled while in ~s mode", [
                    Rtd, ?T_MODE_DIRECT
                ])
            )
        end
...
-spec do_handle(PidSrc, State, OnForward, OnHandle) -> term() when
    PidSrc :: pid(),
    State :: state(),
    OnForward :: fun((NextHop :: pid()) -> term()),
    OnHandle :: fun(() -> term()).
do_handle(PidSrc, #state{routes = Routes, traced = Traced}, OnForward, OnHandle) when
    is_function(OnHandle, 0), is_function(OnForward, 1)
->
    % TODO: Update docs.
...
%% {@returns Updated tracer state, or does not return if tracer is garbage
%%           collected.}
-spec try_gc(State, Analyzer, Parent) -> state() | no_return() when
    State :: state(),
    Analyzer :: analyzer(),
    Parent :: parent().
...
-spec route(PidT, Msg) -> routed(event:evm_event()) | routed(detach()) when
    PidT :: pid(),
...
-spec forwd(PidT, Rtd) -> routed(event:evm_event()) | routed(detach()) when
    PidT :: pid(),
...
-spec add_proc(PidS, TMode, Traced) -> traced() when
    PidS :: pid(),
    TMode :: t_mode(),
    Traced :: traced().
add_proc(PidS, TMode, Traced) ->
    ?assertNot(
        maps:is_key(PidS, Traced),
...
del_proc(PidS, Traced) ->
    ...
    ?TRACE("Process ~w deleted from traced processes map while in ~w mode.", [
        PidS, maps:get(PidS, Traced)
    ]),
...
%%
%% {@returns Updated traced processes map.}
-spec upd_proc(PidS, NewMode, Traced) -> traced() when
    PidS :: pid(),
    NewMode :: t_mode(),
    Traced :: traced().
upd_proc(PidS, NewMode, Traced) ->
    ...
    case maps:is_key(PidS, Traced) of
        true ->
            % Only update existing value, otherwise fail.
            Traced#{PidS := NewMode};
        false ->
...
-spec upd_stats(Stats, Evt) -> stats() when
    Stats :: stats(),
    Evt :: tuple().
...
%%
%% {@returns Updated tracer state.}
-spec upd_state(State, Evt) -> state() when
    State :: state(),
    Evt :: event:evm_event().
...
%% }
-spec new_analyzer(AnlFun, Parent, AMode) -> pid() | self when
    AnlFun :: analyzer:monitor(),
    Parent :: parent(),
    AMode :: a_mode().
...
-spec analyze(Analyzer, Evt) -> Evt when
    Analyzer :: analyzer(),
    Evt :: event:evm_event().
...
analyze(PidA, Evt) when is_pid(PidA) ->
    ?TRACE(
        "Tracer ~w ~s event ~w to ~w for external analysis.",
        [
            self(),
...
show_state(State, Data) ->
    ...
    S0 =
        color_by_pid(self(), format("~n~64c[ ~s ]~64c~n", [$-, Title, $-])) ++
            ...
            color_by_pid(
                self(), format("~" ++ integer_to_list(length(Title) + (64 * 2) + 4) ++ "c~n", [$-])
            ),
