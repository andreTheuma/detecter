...
visit_nec(_Node = {nec, _, Act, Shml}, Opts) ->
    ...
    % !CHECK WHY THE CORRUPT ACTION IS NOT BEING VISITED WHEN A CORRUPT ACTION IS DETECTED...
...
-spec generate_corrupt_clause(Nec, Opts) -> erl_syntax:syntaxTree()
when
    Nec :: af_nec(),
    Opts :: options().
generate_corrupt_clause(_Node = {nec, _, _, _}, Opts) ->
    EventVar = erl_syntax:variable('_E'),
