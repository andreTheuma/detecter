...
-type af_act() ::
    af_fork_act() | af_init_act() | af_exit_act() | af_send_act() | af_recv_act() | af_user_act().
...
-spec compile(File, Opts) -> ok | {error, Reason} when
    File :: file:filename(),
    Opts :: options(),
...
-spec create_module(Ast, Module, Opts) -> Forms :: [erl_parse:abstract_form()] when
    Ast :: [formula()],
    Module :: module(),
    Opts :: options().
...
-spec visit_forms(Forms, Opts) -> Forms :: [erl_syntax:syntaxTree()] when
    Forms :: [formula()],
    Opts :: options().
visit_forms([], Opts) ->
...
%% }
-spec visit_shml(Shml, Opts) -> erl_syntax:syntaxTree() when
    Shml :: af_shml(),
    Opts :: options().
...
visit_shml(Var = {?HML_VAR, _, Name}, Opts) ->
...
visit_shml(_Node = {?HML_MAX, _, Var = {?HML_VAR, _, _}, Shml}, Opts) ->
...
visit_shml(_Node = {'and', _, _, ShmlSeq}, Opts) when is_list(ShmlSeq) ->
    ...
    % Create a function clause that conjuncts all the corrupt action prefixes together,
    % denoting the monitor choice between said corrupt actions.
    CorruptClause = visit_corrupt_seq(ShmlSeq, Opts),
    ...
    erl_syntax:fun_expr(Clauses ++ CorruptClause ++ CatchAllClause).

%% @private Generates a sequence of syntax trees for function clauses denoting
%% all the corrupt action prefixes that the monitor then chooses from.
% -spec build_corrupt_seq() -> [erl_syntax:syntaxTree()].
% build_corrupt_seq() ->
...
-spec visit_shml_seq(ShmlSeq, Opts) -> [erl_syntax:syntaxTree()] when
    ShmlSeq :: af_shml_seq(),
    Opts :: options().
...
%% @private Generates the syntax tree for a function clause describing corrupt
%% action prefixes.
-spec visit_corrupt_seq(ShmlSeq, Opts) -> [erl_syntax:syntaxTree()] when
    ShmlSeq :: af_shml_seq(),
    Opts :: options().
visit_corrupt_seq([Nec], Opts) ->
    [generate_corrupt_clause(Nec, Opts)];
visit_corrupt_seq([Nec | ShmlSeq], Opts) ->
    [generate_corrupt_clause(Nec, Opts) | visit_corrupt_seq(ShmlSeq, Opts)].
...
-spec visit_nec(Nec, Opts) -> erl_syntax:syntaxTree() when
    Nec :: af_nec(),
    Opts :: options().
...
visit_act({fork, _, Var0, Var1, {mfa, _, Mod, Fun, _, Clause}}) ->
    Patterns = erl_syntax:clause_patterns(Clause),
    Guard = erl_syntax:clause_guard(Clause),
    Mfa = erl_syntax:tuple([
        erl_syntax:atom(Mod),
        erl_syntax:atom(Fun),
        erl_syntax:list(Patterns)
    ]),
    Pattern = erl_syntax:tuple([
...
visit_act({user, _, Clause}) ->
    Log = create_log("User-defined action ~p.~n", [Clause], var),
    {erl_syntax:clause_patterns(Clause), erl_syntax:clause_guard(Clause)}.

%% @private Unconditionally generates the syntax tree for a function clause that
%% logs the detection of a corrupt event. The clause is combined together with other
%% such clauses (resulting from other necessities) under one function that describes
%% a monitor choice between said necessities.
-spec generate_corrupt_clause(Nec, Opts) -> erl_syntax:syntaxTree() when
    Nec :: af_nec(),
    Opts :: options().
generate_corrupt_clause(_Node = {nec, _, _, _}, _) ->

    EventVar = erl_syntax:variable('_E'),
    
    EventTable = visit_event_table(),
    PosssibleEvents = erl_syntax:application(
        erl_syntax:atom(element),
        [erl_syntax:integer(3), EventTable]
    ),
    % EventExpr = erl_syntax:list()

    Guard =
        erl_syntax:infix_expr(
            erl_syntax:application(
                erl_syntax:atom(element),
                [erl_syntax:integer(1), EventVar]
            ),
            erl_syntax:operator('=:='),
            erl_syntax:atom(corrupt_payload)
        ),

    Log = create_log("Corrupt event ~p detected.~n", [EventVar], corrupt),
    Body = erl_syntax:block_expr([Log | [erl_syntax:atom(corrupt)]]),
...
%% @private Visits the event table and generates a syntax tree that can be used by the 
%% monitor to determine the next event to transition to. The event table is used to
%% determine the next event to transition to based on the current event. It is only needed when the current 
%% event is corrupt.
-spec visit_event_table() -> erl_syntax:syntaxTree().
visit_event_table() -> 
    #{corrupt_payload => [current_event, previous_event, next_event]}.
...
-spec create_log(Format, Args, Type) -> Call :: erl_syntax:syntaxTree() when
    Format :: string(),
    Args :: [erl_syntax:syntaxTree()],
    Type :: no | prf | var | 'end' | corrupt.
...
-spec color_log(Log, Type) -> Log0 :: string() when
    Log :: iolist(),
    Type :: no | prf | var | 'end' | corrupt.
color_log(Log, no) ->
    % Bold red.
...
color_log(Log, corrupt) ->
    % Bold magenta.
    lists:flatten(["\e[1m\e[35m", Log, "\e[0m"]);
color_log(Log, _) ->
    lists:flatten(Log).
...
-spec write_monitor(Ast, File, Opts) -> ok | {error, Reason} when
...
-spec list_erl(IoDev, Forms) -> ok when
    IoDev :: file:io_device(),
...
-spec list_beam(IoDev, Beam) -> ok | {error, Reason} when
    IoDev :: file:io_device(),
    Beam :: binary(),
    Reason :: file:posix() | badarg | terminated.
...
%% error occurred.
-spec format_error(ErrorInfo) -> {Line :: line(), Description :: string()} when
