...
%%% Public API.
-export([weave/3, weave_file/3,load_mods/1]).
...
weave_remote_spawn({call, Line, {remote, _, ModSpawn = {atom, _, erlang}, FunSpawn = {atom, _, How}}, SpawnArgs}, MfaSpec, FilterSpec, Id)
  ...
  PutCall = create_embed(Line, MonFunVar),
...
create_embed(Line, MonFun) ->
  abs_remote_call(Line, abs_atom(Line, ?TRACE_MOD), abs_atom(Line, embed),
    [MonFun]).
