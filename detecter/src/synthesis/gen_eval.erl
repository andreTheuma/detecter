...
%%% Public API.
-export([create_log/3, pat_tuple/1, corrupted_pat_tuple/1]).
...
% Monitor table option.
-define(OPT_MONITOR_TABLE, mtab).
...
-type af_pattern() :: af_fork() | af_init() | af_exit() | af_send() | af_recv() | af_corr().
...
-type af_corr() :: {corrupt_payload, af_send()} | {corrupt_payload, af_recv()}.
...
pat_tuple(Pat) when element(1, Pat) =:= corrupt_payload ->
    erl_syntax:tuple([
        erl_syntax:atom(corrupt_payload), erl_syntax:underscore()
    ]).

%% @private Translates corrupted action patterns to a native corrupted event pattern.
%% This is used to match corrupted events in the monitor.
-spec corrupted_pat_tuple(Variable) -> erl_syntax:syntaxTree() when
    Variable :: string().
corrupted_pat_tuple(Variable) ->
